...
namespace UdemyCarBook.WebUI.Areas.Admin.Controllers
{
    ...
    public class AdminAboutController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var respoonseMessage = await client.GetAsync("https://localhost:7113/api/Abouts");
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"https://localhost:7113/api/Abouts?id={id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index", "AdminAbout", new { area = "Admin" });
            }
        ...
        [HttpPost]
        [Route("UpdateAbout/{id}")]
        public async Task<IActionResult> UpdateBanner(UpdateAboutDto updateAboutDto)
        {
            var client = _httpClientFactory.CreateClient();
